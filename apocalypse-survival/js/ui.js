...
const UI = {
  ...
  onChoice(text, btnEl) {
    ...
    if (it.kind === 'weekend') { if (s.ap <= 0) this.weekend(); else this.toast('还有行动力，先把这周过完。'); return; }
    if (it.kind === 'hint') { this.autoUse(it.what); return; }   // 直接用最合适的物品，结果显示在正文
    if (it.kind === 'action') {
  ...
  autoUse(category) {
    ...
    if (!best) { this.toast(category === 'med' ? '背包里没有医疗物品' : '背包里没有食物'); return; }
    this.useCarried(best.name);
  },
  ...
  renderPanel() {
    ...
    this.el('panel').innerHTML = `
      ...
      <div class="sec"><h3>属性</h3><div class="attrs">${attrs}</div></div>
      ...
      <div class="sec"><h3>携带物品 <span class="dim">(${s.inventory.length}/${Engine.carryCap()})</span></h3>
        <ul class="inv carried">${carried}</ul></div>
